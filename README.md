...
Therefore, it is important to estimate $\Delta R$ for dating tools that rely on radiocarbon ages. In this work, I update the values of $\Delta R$ according to the calibrated curve (Marine20 & Shcal20), reducing interhemispheric $^{14}C$ gradient between noth to south hemisphere (Hogg et al.,2020). Additionally, I focus on the relationship between $\Delta R$ and space-time variables (latitude, longitude, calibrated age, and age uncertainty), and whether the hypotheses that were proposed in previous works still hold true when using Marine20.
...
### Factorial Principal components
